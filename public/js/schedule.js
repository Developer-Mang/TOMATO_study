...
const $scheduleList = document.querySelectorAll(
  "#schedule ul.scheduleList div.halfMinute"
);
...
const filterTodos = (_todos) =>
  _todos.filter((todo) => todo.date === $selectDate.value);
...
const insertValue = (todo) => {
  ...
  const $startDate = $editScheTodo.querySelector(
    'li.startDate > input[type="date"]'
  );
...
const _getContent = () =>
  $editScheTodo.querySelector(".addInput > .todoInput input").value;

const _getGoal = () =>
  +$editScheTodo.querySelector(".addInput .category select").value;
...
const _getImp = () =>
  $editScheTodo
    .querySelector("li.impSelect .btnImp")
    .classList.contains("impCheck");
...
const _getDetail = () =>
  $editScheTodo.querySelector("li.contentInput > textarea").value;
...
$editScheTodo.onclick = (e) => {
  ...
  if (e.target.matches(".editShecTodo li.impSelect .btnImp"))
