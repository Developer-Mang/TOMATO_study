...
const resetAddGoal = () => {
  ...
  if ($addGoalColor.querySelector("input:checked"))
    $addGoalColor.querySelector("input:checked").checked = false;
};
...
const editGoalFn = async (id) => {
  ...
  if (
    checkDday &&
    new Date($editGoalDday.value) - new Date(generateDate(now)) < 0
  ) {
    openAlert("목표 D-day를 오늘 이후로 선택하십시요.");
    return;
  }
  ...
  if (
    ...
    goals.some(
      (goal) =>
        goal.color === $editGoalColor.querySelector("input:checked").value
    )
  ) {
