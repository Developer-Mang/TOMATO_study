...
const $weeklyAchievementGoalTime = document.querySelector(
  ".weekly .achievement span"
);
...
const secondsToHours = (sec) => {
  ...
  time = `${hour}시간 ${minute < 10 ? "0" : ""}${minute}분 ${
    seconds < 10 ? "0" : ""
  }${seconds}초`;
  return time;
};
...
const getDailyDoneSeconds = (date) => {
  ...
  todos.forEach((todo) => {
    ...
    let changeTarget = splitTarget.split(":");
    changeTarget =
      changeTarget[0] * 60 * 60 + changeTarget[1] * 60 + +changeTarget[2];
    sumTemp += changeTarget;
  });
  return sumTemp;
};
...
const weeklyChallengeRender = () => {
  ...
  week.forEach((day) => {
    ...
    const sucessCriteria = () =>
      `${rateCalc(doneSeconds, goalSeconds)}` > 30
        ? `${rateCalc(doneSeconds, goalSeconds)}` > 80
          ? "perfect"
          : "seccess"
        : "fail";
    const getBtn = () =>
      `${rateCalc(doneSeconds, goalSeconds)}` > 30
        ? `${rateCalc(doneSeconds, goalSeconds)}` > 80
          ? '<button class="stamp">성공</button>'
          : '<button class="stamp">통과</button>'
        : '<button class="stamp">실패</button>';
    const parseToday = Date.parse(getToday());
    const parseDayDate = Date.parse(day.date);
    const getDate = day.date.split("-")[2];

    weeklyChallengeHtml += `<div id="${day.id}" class="${sucessCriteria(
      `${rateCalc(doneSeconds, goalSeconds)}`
    )}">
    <dt>${day.dayName}</dt>
    <dd>${
      parseToday > parseDayDate
        ? getBtn(`${rateCalc(doneSeconds, goalSeconds)}`)
        : ""
    }${getDate}</dd>
  </div>`;
  });
...
const dailyDataRender = (date) => {
  ...
  <li class="achievement">
    <h4>달성률 <span>목표 : ${secondsToHours(goalSeconds).slice(
      0,
      -4
    )}</span></h4>
    <p>${
    ...
    <div class="progress">
      <div class="progressBar" style="width:${rateCalc(
        doneSeconds,
        goalSeconds
      )}%;"></div>
    </div>
    ...
    <ul class="graph">
      <li class="red">
        ...
        <div class="bar" style="height: ${
          getIndexRate(date, "red") ? getIndexRate(date, "red") : "0"
        }%">
          <span class="percent">${
            getIndexRate(date, "red") ? getIndexRate(date, "red") : "0"
          }%</span>
      ...
      <li class="yellow">
        ...
        <div class="bar" style="height: ${
          getIndexRate(date, "yellow") ? getIndexRate(date, "yellow") : "0"
        }%">
          <span class="percent">${
            getIndexRate(date, "yellow") ? getIndexRate(date, "yellow") : "0"
          }%</span>
      ...
      <li class="green">
        ...
        <div class="bar" style="height: ${
          getIndexRate(date, "green") ? getIndexRate(date, "green") : "0"
        }%">
          <span class="percent">${
            getIndexRate(date, "green") ? getIndexRate(date, "green") : "0"
          }%</span>
      ...
      <li class="blue">
        ...
        <div class="bar" style="height: ${
          getIndexRate(date, "blue") ? getIndexRate(date, "blue") : "0"
        }%">
          <span class="percent">${
            getIndexRate(date, "blue") ? getIndexRate(date, "blue") : "0"
          }%</span>
      ...
      <li class="purple">
        ...
        <div class="bar" style="height: ${
          getIndexRate(date, "purple") ? getIndexRate(date, "purple") : "0"
        }%">
          <span class="percent">${
            getIndexRate(date, "purple") ? getIndexRate(date, "purple") : "0"
          }%</span>
...
const graphRender = () => {
  ...
  <li class="red">
    <label>${getGoalTitle("red")}</label>
    <div class="bar" style="height: ${
      getIndexWeeklyRate("red") ? getIndexWeeklyRate("red") : "0"
    }%">
      <span class="percent">${
        getIndexWeeklyRate("red") ? getIndexWeeklyRate("red") : "0"
      }%</span>
  ...
  <li class="yellow">
    <label>${getGoalTitle("yellow")}</label>
    <div class="bar" style="height: ${
      getIndexWeeklyRate("yellow") ? getIndexWeeklyRate("yellow") : "0"
    }%">
      <span class="percent">${
        getIndexWeeklyRate("yellow") ? getIndexWeeklyRate("yellow") : "0"
      }%</span>
  ...
  <li class="blue">
    <label>${getGoalTitle("blue")}</label>
    <div class="bar" style="height: ${
      getIndexWeeklyRate("blue") ? getIndexWeeklyRate("blue") : "0"
    }%">
      <span class="percent">${
        getIndexWeeklyRate("blue") ? getIndexWeeklyRate("blue") : "0"
      }%</span>
  ...
  <li class="green">
    <label>${getGoalTitle("green")}</label>
    <div class="bar" style="height: ${
      getIndexWeeklyRate("green") ? getIndexWeeklyRate("green") : "0"
    }%">
      <span class="percent">${
        getIndexWeeklyRate("green") ? getIndexWeeklyRate("green") : "0"
      }%</span>
  ...
  <li class="purple">
    <label>${getGoalTitle("purple")}</label>
    <div class="bar" style="height: ${
      getIndexWeeklyRate("purple") ? getIndexWeeklyRate("purple") : "0"
    }%">
      <span class="percent">${
        getIndexWeeklyRate("purple") ? getIndexWeeklyRate("purple") : "0"
      }%</span>
...
const render = () => {
  $period.textContent = `${dateChangeForm2(week[0].date)} ~ ${dateChangeForm2(
    week[6].date
  )}`;
...
$statsContent.onclick = ({ target }) => {
  ...
  if (target.parentNode.classList.contains("weekly")) {
    ...
    $weeklyAchievementGoalTime.textContent = `목표 : ${secondsToHours(
      goalSeconds
    ).slice(0, -4)}`;
...
window.onload = () => {
  ...
  document
    ...
    .forEach((input) => (input.min = generateDate(now)));
  // 시간 선택 최소 최대 값 설정
